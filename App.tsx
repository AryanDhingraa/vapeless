...
const STORAGE_KEY_PUFFS = 'vapeless_puffs';
const STORAGE_KEY_SETTINGS = 'vapeless_settings';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const savedPuffs = localStorage.getItem(STORAGE_KEY_PUFFS);
    const savedSettings = localStorage.getItem(STORAGE_KEY_SETTINGS);
    if (savedPuffs) setPuffs(JSON.parse(savedPuffs));
  ...
  // Widget Mode UI (iOS Lockscreen / Quick Action Simulation)
  if (isWidgetMode) {
    ...
    return (
      <div className="fixed inset-0 bg-black text-white flex flex-col items-center justify-center p-8 z-[1000] animate-in fade-in duration-300">
        <button 
          ...
          className="absolute top-12 right-6 text-white/50 font-black text-xs border border-white/20 px-3 py-1 rounded-full"
        >
          CLOSE_WIDGET
        </button>
        
        <div className="text-center space-y-2 mb-12">
          <div className="text-[10px] font-black tracking-widest opacity-50 uppercase">DAILY_LOG_COUNT</div>
          <div className="text-8xl font-black tracking-tighter">{todayCount}</div>
          <div className="text-[10px] font-black bg-white text-black px-2 py-0.5">SECURE_ACTIVE</div>
        </div>
        ...
        <button 
          ...
          className={`
            ...
            ${isAnimatePuff ? 'bg-white' : ''}
          `}
        >
          <span className={`text-4xl font-black uppercase ${isAnimatePuff ? 'text-black' : 'text-white'}`}>
            {isAnimatePuff ? 'LOGGED' : 'PUFF'}
          </span>
        </button>

        <div className="mt-12 text-[10px] font-black opacity-30 text-center italic">
          TAP TO LOG PUFF INSTANTLY.<br/>DATA SYNCED TO LOCAL STORAGE.
        </div>
      </div>
    );
  }
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'settings': return <Settings settings={settings} setSettings={saveSettings} onClearData={() => {
        if(confirm('SYSTEM WIPE: DELETE ALL DATA?')) { setPuffs([]); setSettings(null); localStorage.clear(); }
      }} />;
    }
  };
      ...
      <div className="w-full max-w-md h-screen relative flex flex-col overflow-hidden">
        ...
        <header className="sticky top-0 z-50 bg-white border-b-4 border-black px-6 py-4 flex items-center justify-between shadow-[0px_4px_0px_rgba(0,0,0,1)]">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-black tracking-tighter uppercase italic">VapeLess v1.2</h1>
          </div>
          <div className="flex gap-4 items-center">
            <button 
              onClick={() => setIsWidgetMode(true)} 
              className="text-[10px] font-black border-2 border-black px-2 py-1 active:bg-black active:text-white"
            >
              WIDGET
            </button>
        ...
        {activeTab === 'dash' && (
          <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-40">
            ...
            >
              ...
              <span className="text-[9px] font-black uppercase">LOG_PUFF</span>
            </button>
          </div>
        )}
